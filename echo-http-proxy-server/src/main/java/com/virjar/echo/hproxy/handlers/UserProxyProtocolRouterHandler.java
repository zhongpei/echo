package com.virjar.echo.hproxy.handlers;


import com.virjar.echo.hproxy.EchoHttpProxyServer;
import com.virjar.echo.hproxy.handlers.http.HttpProxyRequestChannelHandler;
import com.virjar.echo.hproxy.handlers.socks.SocksServerHandler;
 ...
 * @date 2021/1/19
 * <p>
 * 判断发送的请求为http(s) 协议还是 socks 协议
 */

@Slf4j
public class UserProxyProtocolRouterHandler extends ChannelInboundHandlerAdapter {

    private final EchoHttpProxyServer echoHttpProxyServer;

    private static final String name = "ECHO_PROTOCOL_FILTER";
    ...
    public UserProxyProtocolRouterHandler(EchoHttpProxyServer echoHttpProxyServer) {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (!(msg instanceof ByteBuf)) {
            log.error(UserProxyProtocolRouterHandler.class.getName() + ": can only handle ByteBuf message");
            ctx.close();
            return;
        }
    ...
    private void route2Socks5(ChannelPipeline pipeline) {
        pipeline.addAfter(name, "SOCKS_INIT_REQUEST_DECODER", new SocksInitRequestDecoder());
        pipeline.addAfter("SOCKS_INIT_REQUEST_DECODER", "SOCKS_MESSAGE_ENCODER",
                new SocksMessageEncoder()
        );
        pipeline.addLast(SocksServerHandler.getName(),
                new SocksServerHandler(this.echoHttpProxyServer)
        );
    }

    private void route2Http(ChannelPipeline pipeline) {
        pipeline.addAfter(name, "encoder", new HttpResponseEncoder());
        pipeline.addAfter(name, "decoder", new HttpRequestDecoder(
                MAX_INITIAL_LINE_LENGTH_DEFAULT,
                MAX_HEADER_SIZE_DEFAULT,
                MAX_CHUNK_SIZE_DEFAULT));
        pipeline.addLast("HTTP_PROXY_REQUEST",
                new HttpProxyRequestChannelHandler(this.echoHttpProxyServer)
        );
    }
