...
public class ProxyUtils {
     ...
     */
    static HttpResponse copyMutableResponseFields(
     ...
     * from { io.netty.handler.codec.http.HttpObjectDecoder#isContentAlwaysEmpty(HttpMessage)}.
     *
     ...
     */
    static boolean isResponseSelfTerminating(HttpResponse response) {
     ...
     */
    private static List<String> getAllCommaSeparatedHeaderValues(String headerName, HttpMessage httpMessage) {
     ...
     */
    static HttpResponse duplicateHttpResponse(HttpResponse originalResponse) {
...
//    }
     ...
     * @return
     */
    public static String getEchoTraceFromUserAgent(HttpRequest httpRequest) {
        ...
        for (Map.Entry<String, String> map : entries) {
            if (map.getKey().equals("User-Agent")) {
                String tmpUA = map.getValue();
                if (tmpUA.contains("traceid")) {
                    UAString = tmpUA;
                }
                break;
            }
     ...
     * @return
     */
    public static HttpRequest ModifyUserAgentFromHttpRequest(HttpRequest httpRequest) {
        String commonUA = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/535.1 (KHTML, like Gecko) Chrome/14.0.835.163 Safari/535.1\n";
        List<Map.Entry<String, String>> entries = httpRequest.headers().entries();
        for (Map.Entry<String, String> map : entries) {
            if (map.getKey().equals("User-Agent")) {
                map.setValue(commonUA);
                break;
            }
        }
        return httpRequest;
